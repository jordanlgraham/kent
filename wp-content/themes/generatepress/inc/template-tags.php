 ...
 * @since 1.3.28
 */
function generate_construct_title_logo()
{
  $generate_settings = wp_parse_args( 
    get_option( 'generate_settings', array() ), 
    generate_get_defaults() 
  ...
  if ( false == $disable_title || false == $disable_tagline ) : 
    ...
    // Print our HTML
      ...
      <?php if ( false == $disable_title ) : ?>
        <div id="site-title"><p class="main-title" itemprop="headline"><a href="<?php echo apply_filters( 'generate_site_title_href', esc_url( home_url( '/' ) ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></p></div>
      <?php endif;
